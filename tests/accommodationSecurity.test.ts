...
const MOCK_RESERVATIONS = { id: 'u6', role: 'Reservations' as UserRole, name: 'User' };
...
describe('Accommodation Security & Role Enforcement', () => {
  ...
  test('Operations cannot access raw supplier rates', () => {
    expect(canAccessRateData(MOCK_OPS.role)).toBe(false);
  });

  test('Reservations can access raw and negotiated supplier rates', () => {
    expect(canAccessRateData(MOCK_RESERVATIONS.role)).toBe(true);
    expect(canAccessNegotiatedRates(MOCK_RESERVATIONS.role)).toBe(true);
  });
  ...
  test('Founder/Admin/Accounts/Reservations can access negotiated rates', () => {
  ...
  test('Server hides supplier-facing internal notes from Operations', () => {
    const mockProp: AccommodationProperty = {
      id: 'p1',
      name: 'Test Hotel',
      propertyType: 'HOTEL',
    ...
    const sanitized = sanitizePropertyForRole(mockProp, MOCK_OPS.role);
    expect(sanitized.internalNotes).toBeUndefined();
  });

  test('Server preserves supplier-facing internal notes for Reservations', () => {
    const mockProp: AccommodationProperty = {
      id: 'p1',
      name: 'Test Hotel',
      propertyType: 'HOTEL',
      ...
      status: 'ACTIVE',
      currency: 'INR',
      internalNotes: 'SUPPLIER CONFIRMATION DETAILS'
    };

    const preserved = sanitizePropertyForRole(mockProp, MOCK_RESERVATIONS.role);
    expect(preserved.internalNotes).toBe('SUPPLIER CONFIRMATION DETAILS');
  });
});
