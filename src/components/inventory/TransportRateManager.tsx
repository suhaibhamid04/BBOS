...
export const TransportRateManager: React.FC = () => {
  const { transportRatePeriods, addTransportRatePeriod, deleteTransportRatePeriod } = useData();
  const { permissions } = useAuth();

  const handleAddMockRate = async () => {
    try {
      await addTransportRatePeriod({
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-lg font-bold text-slate-900">Transport Rates</h2>
          <p className="text-sm text-slate-500">Manage pricing and availability periods.</p>
        </div>
          ...
          <div key={rate.id} className="bg-white border border-slate-200 rounded-xl p-5 hover:border-[#7056EE]/30 transition-colors">
            <div className="flex items-start justify-between mb-3">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="font-bold text-slate-900">Vehicle: {rate.vehicleCategoryId}</h3>
              ...
              {permissions.canManageOperations && (
                <div className="flex items-center gap-1">
                  ...
                  <button 
                    onClick={() => deleteTransportRatePeriod(rate.id)}
            ...
            <div className="grid grid-cols-2 gap-3 mb-4">
              <div className="bg-slate-50 rounded-lg p-2 flex flex-col justify-center">
                <span className="text-[10px] uppercase font-bold text-slate-400">Base Rate</span>
                <span className="font-semibold text-sm text-slate-900">{rate.currency} {rate.baseRate}</span>
              </div>
              <div className="bg-slate-50 rounded-lg p-2 flex flex-col justify-center">
                <span className="text-[10px] uppercase font-bold text-slate-400">Status</span>
                <span className="font-semibold text-sm text-emerald-600">{rate.availabilityStatus}</span>
              </div>
            </div>
